...
from src.blockchain.publisher import publish_batch_anchor, publish_batch_anchor_by_id
from src.blockchain.verifier import verify_on_chain_anchor, verify_on_chain_anchor_by_id
...
def test_publish_batch_anchor_by_id_updates_exact_anchor():
    engine = create_engine("sqlite:///:memory:", future=True)
    Base.metadata.create_all(engine)
    ...
    with Session(engine) as db_session:
        ...
        result = publish_batch_anchor_by_id(
            anchor_id,
            db_session=db_session,
            config=_config(),
            sender=sender,
        ...
        stored = db_session.get(BatchAnchor, anchor_id)

    assert result["anchor_id"] == anchor_id
    assert stored.chain_tx == TX_HASH
...
def test_verify_on_chain_anchor_by_id_persists_match_result():
    engine = create_engine("sqlite:///:memory:", future=True)
    Base.metadata.create_all(engine)
    ...
    with Session(engine) as db_session:
        ...
        result = verify_on_chain_anchor_by_id(
            anchor_id,
            db_session=db_session,
            config=_config(),
            reader=reader,
    ...
    assert result["match"] is True
    assert verification.verification_type == "on_chain_batch"
    assert verification.details_json["anchor_id"] == anchor_id
